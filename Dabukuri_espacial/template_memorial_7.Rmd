...
<header>
<style>
<!-- body{ align: center; text-size:14px; size: A4; margin: 0px; padding:2px; width: 210mm;  height: 297mm; <!--border: 1px solid blue; - ->} -->
@media print {
  @page {
    size: A4;
    text-size:10px;
...
.container {
 ...
 display: flex;
 column-gap: 5px;
 <!--width: 209mm;--><!-- height: 297mm; --><!-- border: 0.2px solid blue; -->
 margin: 1px;
 padding: 1px;
}
</style>
...
resp_tecnico <- "Ivani Ferreira de Faria – Geógrafa (CREAM-AM 123456789-0)"
tipo_de_imovel <- "Rural"
comarca <- '4º Registro de Imóveis de Manaus'
...
<body>
<div class="container" width = '209mm' height = '295mm' style = "border:1 px solid black ">

<!-- HEADER -->
<div class="container" width = "190mm">
<table>   <!-- style = "border:0px"> --> 
<tr>
<td width = 70%> <h3>  Memorial Descritivo  </h2> </td>
<td width = auto > <h3 > DABUKURI </h3> </td>
<td> <img width = 100px src = "figures/DABUKURI.png"> </td>
</tr>
</table> 
</div>
...
padding-left, padding-right:5px; text-align: justified; font-size: 14px;">
    
<h4 style="text-align: left;">  Dados do Imóvel  </h4>
Tipo de Imóvel: __`r toupper(tipo_de_imovel)`__; Uso: __`r toupper(uso)`__;

Proprietário:  __`r V$nome`__ ;  CPF: __`r V$cpf`__; Endereço: __`r paste0(V$rua, ", casa nº", V$id, end, ", Manaus, Brasil")`__; Comarca: __`r comarca`__; Área: __`r V$area`m^2^__; Perímetro: __`r V$perim`m__;
...
<!-- map -->
<td width = 45% >
```{r, out.width=='300px', fig.align='right'}

if ("comunidade" %in% names(tab)) { figure_path <- paste0('figures/', V$comunidade[1], '/', V$comunidade[1], '_P&B_casa_', V$id, '.png') } else { figure_path <- paste0('figures/casa__', V$id, '.png')}
...
__Lateral direita (`r toupper(V$orient_lat_dir)`)__: `r ifelse(!is.na(V$vizinho_dir), paste0( "Confronta-se com o terreno da casa ", V$vizinho_dir, "."), "")` É constituída pela linha imaginária entre os marcos M-02 (`r M2`) e M-03 (`r M3`), com `r V$dist_lateral` metros; `r ifelse ( ! is.na(tab[i,"obs_lat_dir"]), tab[i,"obs_lat_dir"], "")` 
...
__Lateral esquerda (`r toupper(V$orient_lat_esq)`)__:  `r ifelse(!is.na(V$vizinho_esq), paste0(" Confronta-se com o terreno da casa ", V$vizinho_esq, "."), "")` A linha que une os marcos M-04 (`r M4`) e M-01 (`r M1`) com `r V$dist_lateral` metros; `r ifelse ( ! is.na(tab[i,"obs_lat_esq"]), tab[i,"obs_lat_esq"], "")`
...
<p style="text-align: right;  font-size = 12px;"> Manaus, `r Sys.Date() %>% format("%d %b %Y")`. </p>
...
<tr>
<td width= 40% style = "font-size = 12px; border-top:1px solid black;"> <p style= "text-align:center;"> Responsável técnica: `r resp_tecnico` </p></td>
<td width = 9% style = "border-top: 0px"> </td>
<td width=40% style = "font-size = 12px; border-top:1px solid black;"> <p style="font-size:16px; text-align:center;"> Proprietário(a): `r V$nome`</p></td>
</tr>
</table>
</div>
